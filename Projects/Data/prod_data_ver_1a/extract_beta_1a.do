...
forvalues i = 1(1)4{ 
import delimited "Desktop/StructEst_W20/Projects/Data/prod_data_ver_1a/type`i'_new.csv", colrange(1:72) clear /*load simulated data for estimation*/
...
local i_l = 18 + `t' // rename v11 - v20 to l1 - l10
rename v`i_l' l`t'

local i_m = 36 + `t' //rename v21 - v30 to m1 - m10
rename v`i_m' m`t'
...
save "Desktop/StructEst_W20/Projects/stata_1a/prod_data_US.dta", replace 
/*start estimation*/


local dlwprogram = "Desktop/StructEst_W20/Projects/stata_1a/acf_mata_original.do" // local means create a group of values. gen is creating a variable within a dataset

do `dlwprogram' //run this. like sourcing code in R

use "Desktop/StructEst_W20/Projects/stata_1a/prod_data_US.dta", clear 
...
putexcel A`s' = mat(beta_dlw) using "Desktop/StructEst_W20/Projects/stata_1a/acf_replicate", sheet("dgp1_`i'") modify /*save results to excel*/
...
import excel "Desktop/StructEst_W20/Projects/stata_1a/acf_replicate.xlsx",sheet("dgp1_1") clear

// import excel "F:\Research\RR\acf_replicate.xlsx",sheet("dgp1_1") clear
sort C
keep if _n == 1 // keep if current observation number is 1
save "Desktop/StructEst_W20/Projects/stata_1a/result_acf_replicate.dta", replace

local loop = 2

while `loop' < = 4{
//this seems to just be putting all the values from the acf_replicate.xlsx file in cols
import excel "Desktop/StructEst_W20/Projects/stata_1a/acf_replicate.xlsx",sheet("dgp1_`loop'") clear //imports 3 values in columns A, B, C
sort C //is this sorting C column?
keep if _n == 1
save "Desktop/StructEst_W20/Projects/stata_1a/result_temp.dta", replace //isn't this jst replacing the result_temp.dta?

append using "Desktop/StructEst_W20/Projects/stata_1a/result_acf_replicate.dta" //appending?
